...
\item \code{S.CARbym()} - fits the convolution or Besag-York-Mollie (BYM) CAR model outlined in \cite{besag1991}. This model can be implemented with binomial, Gaussian, and Poisson data likelihoods.
...
\item \code{residuals()} - returns either the ``response" (raw) or ``pearson", residuals from the model (based on posterior medians).
\end{itemize}
